...
namespace XPlatHelpers
{
    ...
    // This class rlies on the fact that modules are void* in both, and using the same Windows API names in the Linux Windows.h.
    class unique_module
    {
        HMODULE _hM = nullptr;
    public:
        ...
        void* proc_address(const char* pCStr) { return GetProcAddress(_hM, pCStr); }
        template <typename T> T proc_address(const char* pCStr) { return reinterpret_cast<T>(proc_address(pCStr)); }
    };
}