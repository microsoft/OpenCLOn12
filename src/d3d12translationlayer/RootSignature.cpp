...
namespace D3D12TranslationLayer
{
    ...
    void RootSignatureDesc::GetAsD3D12Desc(VersionedRootSignatureDescWithStorage& Storage, ImmediateContext* pParent) const
    {
        ...
        ShaderStage const& Stage = m_ShaderStages[0];
        ...
        ++ParameterIndex;
        Storage.Parameter[ParameterIndex].InitAsDescriptorTable(m_NumSRVSpacesUsed[0], pSRVRanges, Visibility);
        for (UINT range = 0; range < m_NumSRVSpacesUsed[0]; ++range)
        {
        ...
        if (pParent->ComputeOnly())
        {
            ...
            Storage.Parameter[ParameterIndex].InitAsDescriptorTable(1, &Storage.DescriptorRanges[RangeIndex], Visibility);
            Storage.DescriptorRanges[RangeIndex++].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0, m_NumSRVSpacesUsed[0] + 1, RangeFlags);
        }
        else
        {
            Storage.Parameter[ParameterIndex].InitAsDescriptorTable(1, &Storage.DescriptorRanges[RangeIndex], Visibility);
        ...
        const D3D12_ROOT_SIGNATURE_FLAGS BaseFlags = D3D12_ROOT_SIGNATURE_FLAG_NONE;
